...
interface Ball {
  ...
  console.log(ball); //10
  console.log(sphere); //20
  
  sphere = ball;
  ball = sphere;
  
  console.log("after", sphere); //10
  console.log("after", ball); //10
  ...
  let abc: { id: number; [a: string]: any }; //Note now 'x' can have any name, just that the property should be of type string
  
  abc = { id: 1, fullName: "Zia" }; // Ok, `fullname` matched by index signature
  abc = { id: 1, name7: "Zia2", fullName: "Zia" };
  const newArray123 = Object.keys(abc);
  console.log(
    "🚀 ~ file: step05c_structural_typing_object_literals.ts:71 ~ newArray:",
    newArray123
  );
